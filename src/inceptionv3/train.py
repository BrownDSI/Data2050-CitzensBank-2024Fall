...
# triplets_path = '/Users/hongmingfu/Desktop/Brown University/DATA2050/preprocessed_dataset/preprocessed/InceptionV3/preprocessed_triplets.npy'
# triplets_path = os.path.abspath(os.path.join(current_dir, '../data-preprocessing', '/EfficientNetb0', 'preprocessed_triplets.npy'))
triplets_path = r"..\..\..\preprocessed_dataset\preprocessed\InceptionV3\preprocessed_triplets.npy"
...
def triplets_to_dataloader(triplets, batch_size):
    ...
    # Print the initial shape for debugging
    print("Original anchor shape:", anchors.shape)  # Should be (batch_size, 299, 299, 3) or (batch_size, 3, 299, 299)
    
    # Only transpose if the channels are in the last dimension
    if anchors.shape[-1] == 3:
        anchors = anchors.transpose((0, 3, 1, 2))
        positives = positives.transpose((0, 3, 1, 2))
        negatives = negatives.transpose((0, 3, 1, 2))
    
    # Final confirmation of shapes
    print("Final anchor shape:", anchors.shape)  # Should be (batch_size, 3, 299, 299)
    print("Final positive shape:", positives.shape)
    print("Final negative shape:", negatives.shape)
...
def evaluate(loader, model, beta):
    ...
    with torch.no_grad():
        for anchor, positive, negative in loader:
            print("Anchor shape:", anchor.shape)  # Should be [batch_size, 3, 299, 299]
            print("Positive shape:", positive.shape)  # Expected: [batch_size, 3, 299, 299]
            print("Negative shape:", negative.shape)  # Expected: [batch_size, 3, 299, 299]
