...
def sample_dataset_bhsig260_hindi(data_path, destination_path, num_individuals, seed, number_of_signatures):
    """
    Samples individuals from the BHSig260 Hindi dataset and restructures it to follow a Cedar-like structure.
    ...
    # Get a list of all user directories in the dataset (e.g., '001', '100', etc.)
    user_dirs = [d for d in os.listdir(data_path) if os.path.isdir(os.path.join(data_path, d))]
    
    # Filter users with enough genuine and forged signatures
    eligible_users = []
    for user_id in user_dirs:
        ...
        # Check if the user has enough genuine and forged signatures
    ...
    # Select random individuals from the eligible users
...
if __name__ == "__main__":
    ...
    dest_path = "./preprocessed_dataset/sampled/BHSig260_Dataset_Hindi/"
    num_individuals = 2
    seed = 123
    number_of_signatures = 5

    sample_dataset_bhsig260_hindi(data_path, dest_path, num_individuals, seed, number_of_signatures)
